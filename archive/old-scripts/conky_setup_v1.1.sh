...
# One-click Conky overlay for Xubuntu that shows:
# - burn-in status/phase/drives from /var/lib/hdd_burnin/current_run.json
# - last line of the burn-in SUMMARY log
# - per-drive temps for selected drives (via smartctl)
#
# Run:
#   sudo ./setup_conky_overlay.sh
#
# Optional env:
#   ENABLE_CONKY_SMART_SUDO=1 (default)  -> allows conky to call smartctl via sudo without prompting
...
echo "Start now (as ${TARGET_USER}): conky -c ~/.config/conky/conky.conf"
