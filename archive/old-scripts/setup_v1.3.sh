#!/usr/bin/env bash
# setup.sh (idempotent)
# - Creates burnin group (Option B security) and sets perms for /var/lib/hdd_burnin + logs
# - Installs deps (only if missing)
# - SSH on port 1492 (or SSH_PORT)
# - UFW adds rules (no reset)
...
ensure_ufw_rule() {
  ...
  if ufw_rule_present "$pattern"; then log "UFW rule already present: $pattern"
  else log "Adding UFW rule: $cmd"; ufw $cmd; fi
}
...
# Create TSV/JSON if missing (so other scripts don't fail)
...
' "$RUNS" | while IFS=$'\t' read -r run_id ts phase outcome sn wwn model size_bytes poh realloc pending offline crc smart_health temp_max log_dir; do
  ...
  if [[ "$outcome" == "ABORTED" ]]; then
    txt="ABORTED: run interrupted ${txt}"
  fi
  echo "$st $svc - $txt"
done
...
EOF
  ...
  log "Local check exists; ensuring ABORTED mapping is CRIT..."
  if ! grep -qE 'ABORTED\)\s*echo\s*2' "$lc_path"; then
    [[ -x /usr/bin/perl ]] || fatal "perl missing; cannot patch local check safely."
...
# Ownership/permissions for local check
chown root:root "$lc_path" || true
