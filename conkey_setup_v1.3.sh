...
# Conky overlay that reads:
# - /var/lib/hdd_burnin/current_run.json (group-readable via burnin group)
# - tails the run summary file (also group-readable)
# - shows per-drive temps for selected drives (via sudo smartctl)
...
apt-get update -y
apt-get install -y --no-install-recommends conky-all lm-sensors smartmontools

echo "[INFO] Running sensors-detect (auto)..."
yes | sensors-detect --auto >/dev/null 2>&1 || true

# Ensure group exists + user is member (Option B)
...
if [[ -z "${NIC:-}" ]]; then
  NIC="$(ip -br link | awk '$1!="lo"{print $1; exit}')"
fi
NIC="${NIC:-eth0}"
...
# Managed by setup_conky_overlay.sh
...
\${execi 3 bash -lc '
  ...
  if [[ -r "\$f" ]]; then
    ...
    echo "current_run.json not readable yet (check group membership).";
  fi
'}
...
\${execi 10 bash -lc '
  ...
  if [[ -r "\$f" ]]; then
    ...
    if [[ -n "\$drives" ]]; then
      for dv in \$drives; do
        t=\$(sudo /usr/sbin/smartctl -A "\$dv" 2>/dev/null | awk "/Temperature_Celsius|Airflow_Temperature_Cel|Temperature_Internal/ {print \\$10; exit} /^Temperature:/ {print \\$2; exit} /Current Drive Temperature/ {print \\$4; exit}" | sed -E "s/[^0-9].*//");
        echo "\$dv temp: \${t:-NA}C";
      done
    fi
  fi
...
echo "If Conky can't read current_run.json yet: log out/in (or reboot) so group membership applies."
