...
# - Signal handling cleanup via trap kills active badblocks jobs and exits 130
# - clean_field() sanitizes '|' in addition to tabs/newlines
# - SMART read failures are WARN (not silent PASS), per-drive in Phase 0
# Chatgpt v4.1x
...
# MUST FIX 🔴 (timing): validate SMART_TIMEOUT without die()/SUMMARY dependency
...
cleanup() {
  ...
  exit 130  # Standard exit code for SIGINT/Ctrl-C (and TERM here for simplicity)
}
...
smart_dump_x() {
  # returns 0 if smartctl succeeded, 1 otherwise; writes something useful either way
...
phase0() {
  ...
  # MUST FIX 🔴: per-drive smart_ok flags (multi-drive correctness)
  ...
  for d in "${drives[@]}"; do
    ...
    # WARN if either SMART dump failed for THIS drive
...
# SHOULD FIX 🟡: hardening/audit note in SUMMARY header
{
  ...
  echo "Hardening: timeout-wrapped smartctl temp polling; trap cleanup kills active badblocks + exit 130; clean_field strips |; Phase0 SMART dump WARN is per-drive"
  echo
} > "$SUMMARY"
