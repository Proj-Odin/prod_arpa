...
# Conky overlay that reads /var/lib/hdd_burnin/current_run.json (group-readable via burnin group)
# Optional arg: network interface name (e.g., eno1). If omitted, autodetects.
...
# Optional NIC override via $1 (safe under set -u)
NIC="${1:-}"
if [[ -z "$NIC" ]]; then
...
echo "If Conky can't read current_run.json yet: log out/in (or reboot) so burnin group membership applies."
