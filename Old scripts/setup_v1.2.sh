...
if [[ -n "${CMK_SERVER_IP}" ]]; then
  ensure_ufw_rule "allow from ${CMK_SERVER_IP} to any port 6556 proto tcp comment \"Checkmk agent from server\"" \
                  "6556\/tcp.*ALLOW IN.*${CMK_SERVER_IP}"
else
...
# ---------------- STATE DIR + TSV SAFETY ----------------
log "Ensuring state dir + TSV DBs exist (prevents file-not-found errors)..."
...
if [[ ! -f "$CUR_JSON" ]]; then
  cat > "$CUR_JSON" <<EOF
{"run_id":"","status":"idle","phase":"IDLE","phase_started_at":"","last_update":"","max_temp_c":0,"block_size":0,"max_batch":0,"log_dir":"","summary_path":"","drives_text":"","drives_dev_text":"","temp_max_c":{}}
EOF
  chmod 600 "$CUR_JSON"
  log "Created: $CUR_JSON"
else
  log "Exists: $CUR_JSON"
fi

# ---------------- LOCAL CHECK (burn-in status) ----------------
log "Installing/patching burn-in Checkmk local check (ABORTED -> CRIT + CurrentRun service)..."
...
if [[ ! -f "$lc_path" ]]; then
  ...
  cat > "$lc_path" <<'EOF'
#!/bin/bash
# Checkmk local check: burn-in status from:
# - /var/lib/hdd_burnin/runs.tsv (per-drive latest verdict)
# - /var/lib/hdd_burnin/current_run.json (real-time status/phase)
...
if [[ -r "$CUR" ]]; then
  ...
  echo "$st HDD_Burnin_CurrentRun - status=${status:-?} phase=${phase:-?} drives='${drives:-}' run_id=${run_id:-?} last_update=${last_update:-?} summary=${summary_path:-?}"
else
  echo "1 HDD_Burnin_CurrentRun - current_run.json missing (burn-in script not run yet?)"
fi

# ---- Per-drive latest verdict services ----
if [[ ! -r "$RUNS" ]]; then
...
EOF
  ...
  log "Local check already exists; ensuring ABORTED -> CRIT mapping..."
fi
...
if ! grep -qE 'ABORTED\)\s*echo\s*2' "$lc_path"; then
  ...
  if have perl; then
    ...
    warn "perl not available; cannot auto-patch state_of()."
  fi
fi
