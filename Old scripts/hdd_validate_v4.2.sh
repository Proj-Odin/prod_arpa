...
# - Phase 0: SMART triage (short+conveyance+long) + logs + per-drive outcome
# - Phase B: DESTRUCTIVE badblocks -w (max 4 drives) + temps + SMART before/after + outcome
#
# Hardened behaviors:
# - Temp polling uses timeout to avoid smartctl hangs
# - trap cleanup kills active badblocks jobs and exits 130
# - clean_field() sanitizes '|' in addition to tabs/newlines
# - Phase 0 SMART dump WARN is per-drive (multi-drive correct)
# - NEW: records ABORTED runs into runs.tsv when cleanup triggers (so Checkmk can alert)
#
...
# Validate SMART_TIMEOUT early without die()/SUMMARY dependency
...
# For ABORTED recording
CURRENT_PHASE=""            # "PHASE0" / "PHASEB" / ""
declare -a CURRENT_DRIVES=() # stable by-id strings for the active phase
ABORT_RECORDED="false"

# ------------------ TSV HELPERS ------------------
clean_field() {
  # TSV-safe: remove tabs/newlines and also '|' (helps prevent downstream parsing/injection issues)
  echo "${1:-}" | tr '\t\r\n|' '    ' | sed -E 's/[[:space:]]+/ /g; s/^ //; s/ $//'
}
...
# normal smart_attr (used outside cleanup)
smart_attr() {
...
# ------------------ ABORTED RUN RECORDING ------------------
record_aborted_runs() {
  # Avoid double-recording if trap fires twice
  [[ "$ABORT_RECORDED" == "true" ]] && return 0
  ABORT_RECORDED="true"

  # If no active phase or no drives selected, nothing meaningful to record
  [[ -n "$CURRENT_PHASE" ]] || return 0
  [[ "${#CURRENT_DRIVES[@]}" -gt 0 ]] || return 0

  # Ensure DB exists
  init_db || true

  echo "[ABORT] Recording ABORTED runs for phase=$CURRENT_PHASE drives=${#CURRENT_DRIVES[@]}" | tee -a "$SUMMARY" 2>/dev/null || true

  local d dev sn wwn model sizeb poh realloc pending offline crc tempmax
  for d in "${CURRENT_DRIVES[@]}"; do
    ...
    # Use timed SMART reads so cleanup doesn't hang on a dying disk
    poh="$(smart_attr_timed "$dev" Power_On_Hours)"
    realloc="$(smart_attr_timed "$dev" Reallocated_Sector_Ct)"
    pending="$(smart_attr_timed "$dev" Current_Pending_Sector)"
    offline="$(smart_attr_timed "$dev" Offline_Uncorrectable)"
    crc="$(smart_attr_timed "$dev" UDMA_CRC_Error_Count)"
    tempmax="${TEMP_MAX[$d]:-}"

    db_append_run "$CURRENT_PHASE" "ABORTED" "$sn" "$wwn" "$model" "$sizeb" \
      "$poh" "$realloc" "$pending" "$offline" "$crc" "${tempmax:-}" "$LOG_DIR" || true
  done
}

# ------------------ CLEANUP / TRAP ------------------
cleanup() {
  # record ABORTED runs before killing jobs (so we still know phase/drives)
  record_aborted_runs || true
...
db_drive_exists() { awk -F'\t' -v sn="$1" 'NR>1 && $1==sn {found=1} END{exit(found?0:1)}' "$DRIVES_DB"; }
...
phase0() {
  ...
  for d in "${drives[@]}"; do
    ...
    tempmax="${TEMP_MAX[$d]:-}"
    echo "[PHASE 0 RESULT] $d : POH=$poh Realloc=$realloc Pending=$pending OfflineUnc=$offline UDMA_CRC=$crc Outcome=$outcome TempMax=${tempmax:-NA}" | tee -a "$SUMMARY"
...
phaseB() {
  ...
  for d in "${drives[@]}"; do
    ...
    echo "POH=$poh Realloc=$realloc Pending=$pending OfflineUnc=$offline UDMA_CRC=$crc Outcome=$outcome TempMax=${tempmax:-NA}" | tee -a "$SUMMARY"
    if [[ -s "$bb_bad" ]]; then
...

{
  ...
  echo "Hardening: timeout-wrapped smartctl temp polling; trap cleanup kills active badblocks + exit 130; clean_field strips |; Phase0 SMART dump WARN is per-drive; ABORTED runs recorded on trap"
  echo
} > "$SUMMARY"
