...
#
# Hardened fixes incorporated:
# 1) clean_field() sanitizes '|' in addition to tabs/newlines (prevents downstream parsing/injection issues)
# 2) cleanup handler exits with 130 (standard exit code for SIGINT / Ctrl-C)
# 3) SMART_TIMEOUT validated as a positive integer
# 4) Phase 0 outcome uses both smart_ok_pre and smart_ok_post (WARN if either dump fails)
# 5) SMART_TIMEOUT comment explaining purpose
#
...
# ------------------ VALIDATION ------------------
# Validate SMART_TIMEOUT value (must be positive integer)
if [[ ! "$SMART_TIMEOUT" =~ ^[0-9]+$ ]] || [[ "$SMART_TIMEOUT" -lt 1 ]]; then
  # die uses SUMMARY, which exists already
  die "SMART_TIMEOUT must be a positive integer (got: $SMART_TIMEOUT)"
fi
...
phase0() {
  ...
  smart_ok_pre="true"  # overall flag for this phase (any failure flips it)
  smart_ok_post="true"
  ...
  for d in "${drives[@]}"; do
    ...
    # outcome determination (now uses smart_ok_pre and smart_ok_post)
    outcome="PASS"
    [[ "$realloc" -gt 0 || "$pending" -gt 0 || "$offline" -gt 0 ]] && outcome="FAIL"
    if [[ "$smart_ok_pre" == "false" || "$smart_ok_post" == "false" ]]; then
