#!/usr/bin/env bash
# badblocks_state_update.sh v1.8 - PRODUCTION READY (ORCHESTRATOR-INTEGRATED)
#
# Adds:
# - Reads /var/lib/hdd_burnin/orchestrator_state.json (if present)
# - Enriches per-drive JSON with orchestrator temps + pass/pattern info
# - Exports additional Prometheus metrics
#
...
# Prometheus label escaping: backslash and quote, and flatten newlines
prom_escape(){
  local s="${1:-}"
  s="${s//\\/\\\\}"
  s="${s//\"/\\\"}"
...
load_orchestrator_state(){
  ...
  local tmp
  tmp="$(cat "$ORCH_STATE" 2>/dev/null || true)"
  [[ -n "$tmp" ]] || return 0

  if ! jq empty >/dev/null 2>&1 <<<"$tmp"; then
    log_warn "orchestrator_state.json exists but is invalid JSON (mid-write?)"
    return 0
  fi
  ...
  ORCH_RUN_ID="$(jq -r '.orchestrator.run_id // empty' <<<"$ORCH_JSON" 2>/dev/null || true)"
  ORCH_PHASE="$(jq -r '.orchestrator.phase // empty' <<<"$ORCH_JSON" 2>/dev/null || true)"
  ORCH_STATUS="$(jq -r '.orchestrator.status // empty' <<<"$ORCH_JSON" 2>/dev/null || true)"
  ORCH_PID="$(norm_int "$(jq -r '.orchestrator.pid // 0' <<<"$ORCH_JSON" 2>/dev/null || echo 0)" 0)"

  if (( ORCH_PID > 0 )) && ! kill -0 "$ORCH_PID" 2>/dev/null; then
    ORCH_STALE=1
    log_warn "Orchestrator PID $ORCH_PID not running; orchestrator_state may be stale"
  fi
}

read_progress_file(){
  local prog_file="${1:-}"
  [[ -n "$prog_file" && -f "$prog_file" ]] || return 0
  tail -n1 "$prog_file" 2>/dev/null || true
}

# Parse lines like: pass=2/4 pattern=0x55 status=running
parse_progress_line(){
  local line="${1:-}"
  local pass_cur=0 pass_tot=0 pat="" st=""
  [[ "$line" =~ pass=([0-9]+)/([0-9]+) ]] && { pass_cur="${BASH_REMATCH[1]}"; pass_tot="${BASH_REMATCH[2]}"; }
  [[ "$line" =~ pattern=([^[:space:]]+) ]] && pat="${BASH_REMATCH[1]}"
  [[ "$line" =~ status=([^[:space:]]+) ]] && st="${BASH_REMATCH[1]}"
  echo "${pass_cur}|${pass_tot}|${pat}|${st}"
}
# ----------------------------------------------------------

write_metrics(){
  local t="${METRICS_FILE}.tmp.$$" j u ph pc er sp et ss st av rid otc otm opc opt
  {
    ...
    echo "# HELP badblocks_orchestrator_temp_current_c Current temperature from orchestrator"
    echo "# TYPE badblocks_orchestrator_temp_current_c gauge"
    echo "# HELP badblocks_orchestrator_temp_max_c Maximum temperature from orchestrator"
    echo "# TYPE badblocks_orchestrator_temp_max_c gauge"
    echo "# HELP badblocks_orchestrator_pass_current Current badblocks pass number from orchestrator progress file"
    echo "# TYPE badblocks_orchestrator_pass_current gauge"
    echo "# HELP badblocks_orchestrator_pass_total Total badblocks passes from orchestrator config/progress file"
    echo "# TYPE badblocks_orchestrator_pass_total gauge"
    ...
    for j in "$STATE_DIR"/*.json; do
      ...
      rid="$(jget "$j" .orchestrator.run_id)"
      rid="$(prom_escape "$rid")"

      otc="$(norm_num "$(jget "$j" .orchestrator.temp_current_c)" 0)"
      otm="$(norm_num "$(jget "$j" .orchestrator.temp_max_c)" 0)"
      opc="$(norm_int "$(jget "$j" .orchestrator.pass_current)" 0)"
      opt="$(norm_int "$(jget "$j" .orchestrator.pass_total)" 0)"
      ...
      echo "badblocks_orchestrator_temp_current_c{device=\"$u\",phase=\"$ph\",run_id=\"$rid\"} $otc"
      echo "badblocks_orchestrator_temp_max_c{device=\"$u\",phase=\"$ph\",run_id=\"$rid\"} $otm"
      echo "badblocks_orchestrator_pass_current{device=\"$u\",phase=\"$ph\",run_id=\"$rid\"} $opc"
      echo "badblocks_orchestrator_pass_total{device=\"$u\",phase=\"$ph\",run_id=\"$rid\"} $opt"
    done
  } >"$t"
...
for pid in "${pids[@]}"; do
  ...
  # Phase start detection + transition logging
  ps="$pps"
  ...
  # -------- Per-drive orchestrator enrichment --------
  orch_drive_uid=""
  orch_drive_serial=""
  orch_temp_c="0"
  orch_temp_max="0"
  ...
  if (( ORCH_AVAILABLE == 1 )); then
    orch_entry="$(jq -c --arg dev "$dev" '.drives[$dev] // empty' <<<"$ORCH_JSON" 2>/dev/null || true)"
    if [[ -n "$orch_entry" && "$orch_entry" != "null" ]]; then
      orch_drive_uid="$(jq -r '.unique_id // empty' <<<"$orch_entry" 2>/dev/null || true)"
      orch_drive_serial="$(jq -r '.serial // empty' <<<"$orch_entry" 2>/dev/null || true)"
      orch_temp_c="$(norm_num "$(jq -r '.temp_current_c // 0' <<<"$orch_entry" 2>/dev/null || echo 0)" 0)"
      orch_temp_max="$(norm_num "$(jq -r '.temp_max_c // 0' <<<"$orch_entry" 2>/dev/null || echo 0)" 0)"
      orch_prog_file="$(jq -r '.progress_file // empty' <<<"$orch_entry" 2>/dev/null || true)"

      orch_prog_line="$(read_progress_file "$orch_prog_file")"
      IFS='|' read -r orch_pass_cur orch_pass_tot orch_pat _ < <(parse_progress_line "$orch_prog_line")
      orch_pass_cur="$(norm_int "$orch_pass_cur" 0)"
      orch_pass_tot="$(norm_int "$orch_pass_tot" 0)"
    fi
  fi
  # -----------------------------------------------
  ...
  c="$(jq -n \
    ...
    --arg bo "$out" \
    --argjson e "$er" \
    --arg orch_run_id "$ORCH_RUN_ID" \
    --arg orch_phase "$ORCH_PHASE" \
    --arg orch_status "$ORCH_STATUS" \
    --argjson orch_pid "$ORCH_PID" \
    --argjson orch_stale "$ORCH_STALE" \
    --arg orch_drive_uid "$orch_drive_uid" \
    --arg orch_drive_serial "$orch_drive_serial" \
    --argjson orch_temp_c "$(norm_num "$orch_temp_c" 0)" \
    --argjson orch_temp_max "$(norm_num "$orch_temp_max" 0)" \
    --argjson orch_pass_cur "$orch_pass_cur" \
    --argjson orch_pass_tot "$orch_pass_tot" \
    --arg orch_pat "$orch_pat" \
    --arg orch_prog_file "$orch_prog_file" \
    --arg orch_prog_line "$orch_prog_line" \
    '{
      suite_version:$sv,
      schema_version:$scv,
      ...
      orchestrator:{
        ...
        pid:$orch_pid,
        stale:$orch_stale,
        drive_unique_id:$orch_drive_uid,
        drive_serial:$orch_drive_serial,
        temp_current_c:$orch_temp_c,
        temp_max_c:$orch_temp_max,
        pass_current:$orch_pass_cur,
        pass_total:$orch_pass_tot,
        pattern_observed:$orch_pat,
        progress_file:$orch_prog_file,
        progress_line:$orch_prog_line
      },
  ...
  # Merge with existing + update phase_history
  if [[ -f "$sf" ]]; then
    c="$(jq -s \
      --arg old_phase "$pph" \
      --arg new_phase "$phl" \
