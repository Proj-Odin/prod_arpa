...
BADBLOCKS_PASSES="${BADBLOCKS_PASSES:-4}" # 1..4 patterns
...
if [[ ! "$BADBLOCKS_PASSES" =~ ^[0-9]+$ ]] || [[ "$BADBLOCKS_PASSES" -lt 1 || "$BADBLOCKS_PASSES" -gt 4 ]]; then
  echo "[FATAL] BADBLOCKS_PASSES must be 1..4 (got: $BADBLOCKS_PASSES)" >&2
  exit 1
fi
...
# ------------------ Phase B worker (per-drive; patterns loop) ------------------
run_badblocks_worker() {
  local dev="$1" s="$2" badfile="$3"
  local -a patterns=(0xaa 0x55 0xff 0x00)
  local i
  : > "$badfile"

  for ((i=1; i<=BADBLOCKS_PASSES; i++)); do
    local pat="${patterns[$((i-1))]}"
    local logf="$LOG_DIR/phaseB_badblocks_${s}_pass${i}_${pat}.log"
    echo "[INFO] badblocks pass $i/$BADBLOCKS_PASSES pattern=$pat dev=$dev" >> "$SUMMARY"
...
phaseB() {
  ...
  echo "[INFO] BADBLOCKS_PASSES=$BADBLOCKS_PASSES (reduce for speed if needed)" | tee -a "$SUMMARY"
  echo
  ...
  for d in "${drives[@]}"; do
    ...
    # refuse if matches detected system disks
    for sd in "${sysdisks[@]}"; do
      [[ "$dev" == "$sd" ]] && die "Refusing to test detected system disk: $dev"
    done
  done
  ...
  for d in "${drives[@]}"; do
    ...
    ( run_badblocks_worker "$dev" "$s" "$bb_bad" ) &
    pid=$!
    PIDS["$d"]=$pid
    ACTIVE_PIDS+=("$pid")

    echo "[INFO] badblocks worker started: $d pid=$pid badlist=$bb_bad" | tee -a "$SUMMARY"
  done
